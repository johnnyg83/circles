...
@app.route('/') 
def home():
    johnny_gundo = User(id='johnny_g', email='john.gunderson@yale.edu', name='john gunderson', online=True)
    chrissy_yaodo = User(id='chrissy_y', email='chris.yao@yale.edu', name='chris yao', online=True)
    db.drop_all()
    db.create_all()
    db.session.commit()
